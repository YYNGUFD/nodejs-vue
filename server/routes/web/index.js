module.exports = app=>{
  ...
  router.get('/heroes/init',async (req,res)=>{
    // $$('.hero-nav>li').map((li,i)=>{return { name:li.innerText,heros:$$('li',$$(".hero-list")[i]).map(hero=>{return {name:$$('h3',hero)[0].innerHTML }})}})
    ...
    router.get('/article/:id',async (req,res)=>{
      ...
      var data =await Article.findById(req.params.id).lean();
